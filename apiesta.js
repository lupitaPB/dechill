...
app.get('/ultimodato', async (req, res) => {
  try {
    ...
    if (data.length > 0) {
      const { _id, Altura, Temperatura } = data[0]; // Ajuste aquí para incluir _id si lo necesitas
      res.json({ _id, Altura, Temperatura }); // Ajuste aquí para incluir _id si lo necesitas
    } else {
